...
'Update'						=>	'更新 FluxBB',
'Update message'				=>	'您的 FluxBB 数据库已经过时，继续之前必须升级。如果您是该论坛管理员，请按下列说明完成升级。',
'Note'							=>	'注意：',
'Members message'				=>	'该进程仅供论坛管理员操作。如您不是管理员，请勿担心——论坛马上就会上线！',
'Administrator only'			=>	'该步骤仅供论坛管理员操作！',
'Database password info'		=>	'要执行数据库升级请填写 FluxBB 所使用的数据库密码。如果您不慎忘记了密码，请查看 \'config.php\' 文件中储存的密码。',
...
'Version mismatch error'		=>	'版本不匹配。数据库 \'%s\' 似乎不包含本升级脚本所支持的 FluxBB 数据库架构。',
...
'Enable conversion'				=>	'<strong>启用转换：</strong> 如启用该功能，在更新脚本完成数据库结构的必须更新后，还会将数据库中的所有文字由当前编码转换为UTF-8。如是从 FluxBB 1.2版更新，该转换过程是必须的。',
...
'Error info 1'					=>	'转换某些用户时出现错误。当从 FluxBB 1.2转换时，如果多个用户的名称过于相似，如 "bob" 和 "böb"，可能会出现该错误。',
...
'Click here'					=>	'点击此处',
'Automatic redirect failed'		=>	'自动重定向失败。 %s 以继续 …',
...
'Username too short error'		=>	'用户名长度至少需要2字符。请重新选择(更长的)用户名。',
'Username too long error'		=>	'用户名长度不可超过25字符。请重新选择(更短的)用户名。',
